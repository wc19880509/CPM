...
Basic Image Processing Functions used by Liu Ce
...
//----------------------------------------------------------------------------------
enum InterType{ INTER_NN, INTER_LINEAR };		//插值方法：最近邻/线性
...
public:
	...
	// basic functions
	template <class T>
	static inline T EnforceRange(const T& x, const int& MaxValue) { return __min(__max(x, 0), MaxValue - 1); };	//限制x在0到MaxValue之间

	// Values for L are in the range[0, 100] while a and b are roughly in the range[-110, 110].
	template <class T1, class T2>
	static void BGR2Lab(T1* pSrcImage, T2* pDstImage, int width, int height);		//Added By Chen
	...
	// 两种Resize方法，一种是指定比例，一种是指定结果的大小
	template <class T1, class T2>
	static void ResizeImage(const T1* pSrcImage, T2* pDstImage, int SrcWidth, int SrcHeight, int nChannels,
		float Ratio, InterType type = INTER_LINEAR);

	template <class T1, class T2>
	static void ResizeImage(const T1* pSrcImage, T2* pDstImage, int SrcWidth, int SrcHeight, int nChannels,
		int DstWidth, int DstHeight, InterType type = INTER_LINEAR);
	...
	template <class T1, class T2>
	static void Medianfiltering(const T1* pSrcImage, T2* pDstImage, int width, int height, int nChannels, int fsize);		//Added By Chen

	template <class T1, class T2>
	static void Integral(const T1* pSrcImage, T2* pDstImage, int width, int height, int nChannels);		//Added By Chen

	template <class T1, class T2> // O(1) time box filtering using cumulative sum
	static void BoxFilter(const T1* pSrcImage, T2* pDstImage, int width, int height, int nChannels, int r, bool norm = true);		//Added By Chen
	...
	//---------------------------------------------------------------------------------
	template <class T1, class T2>
	static void cropImage(const T1* pSrcImage, int SrcWidth, int SrcHeight, int nChannels, T2* pDstImage,
		int Left, int Top, int DstWidth, int DstHeight);
...
void ImageProcessing::BGR2Lab(T1* pSrcImage, T2* pDstImage, int width, int height)
{
	...
	for (int i = 0; i < npix; i++){
		...
		float Z = 0.019334 * r + 0.119193 * g + 0.950227 * b;		// RGB to XYZ space
		//|X|		|R|
		//|Y| =	M *	|G|
		//|Z|		|B|
		X /= 0.950456;
		...
		float fX = X > T ? pow(X, 1. / 3) : 7.787f * X + 0.137931f;
		float fY = Y > T ? Y3 : 7.787f * Y + 0.137931f;
		float fZ = Z > T ? pow(Z, 1. / 3) : 7.787f * Z + 0.137931f;

		float L = Y > T ? (116 * Y3 - 16.0) : (903.3f * Y);
		float A = 500 * (fX - fY);
		float B = 200 * (fY - fZ);
...
inline void ImageProcessing::BilinearInterpolate(const T1* pImage, int width, int height, int nChannels, float x, float y, T2* result)
{
	...
	for (m = 0; m <= 1; m++)
	{
		for (n = 0; n <= 1; n++)
		{
			...
			s = fabs(1 - m - dx) * fabs(1 - n - dy);	//这是一种巧妙的写法
